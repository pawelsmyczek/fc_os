...
static __IO uint64_t mpu_timestamp;
...
: spi(_spi)
{
    ...
    delay_ms(150);
    write(MPU6000_PWR_MGMT_1, BIT_H_RESET);
    delay_ms(150);
    write(SIGNAL_PATH_RESET, 0x07);
    write(MPU6000_PWR_MGMT_1, MPU_CLK_SEL_PLLGYROZ);
    write(MPU6000_USER_CTRL, BIT_I2C_IF_DIS);
    write(MPU6000_PWR_MGMT_2, 0x00);
...
//    {
//        spi->spi_enable();
//        spi->spi_transfer(MPU6000_TEMP_OUT_H | 0x80);
//        raw_mpu[0] = spi->spi_transfer(0x00);
//        spi->spi_disable();
//    }
...
void MPU6000_::write(uint8_t reg, uint8_t data)
{
    spi->spi_enable();
...
bool MPU6000_::detect_mpu(void)
{
    ...
    do {
        ...
        const uint8_t whoAmI = spi->spi_transfer(MPU6000_WHOAMI);
...
void MPU6000_::data_transfer_callback_mpu(void) {
...
void data_transfer_callback(void)
{
    mpu->data_transfer_callback_mpu();
}

void MPU6000_::read_mpu_dma(void)
{
    mpu_timestamp = micros();
    raw_mpu[0] = MPU6000_ACCEL_XOUT_H | 0x80;
    spi->dma_transfer(raw_mpu, raw_mpu, 15, &data_transfer_callback);
}
...
void MPU6000_::compute_mpu_tc_bias(void)
{
...
void MPU6000_::compute_angles(void)
{
...
float
MPU6000_::get_accel(uint8_t axis)
{
    return accel_sum[axis];
}

// void create_rotation_matrix(void);
void MPU6000_::movement_end_check()
{}
void MPU6000_::positions_estimate(void)
{}
