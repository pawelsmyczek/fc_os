...
int main(void)
{
    ...
    uint32_t bmp_time_start = 0;
    uint32_t bmp_time_end = 0;
...
//        sprintf(serial_out, "ACCEL[YAW] - %u\n\r", sizeof(float));
        ...
        if(serial_in_buffer == 'b'){
            serial_out_len = sprintf(serial_out, "%.3f, %hd, time spent reading %ld\n\r", bmp_data.altitude_current, bmp_data.temperature_read, (bmp_time_end - bmp_time_start));
            CDC_Send_DATA(serial_out, serial_out_len);
            delay_ms(100);
        }
        if(serial_in_buffer == 'r'){
            if(success_flash_read){
