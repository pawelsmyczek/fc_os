...
//    sample_config_file.crc = crc;
    ...
    CDC_Send_DATA(reinterpret_cast<const uint8_t *>("FLIGHT CONTROL PROGRAM BEGIN - type 'c' to continue\n\r"), 54);
    while(serial_in_buffer != 'c') CDC_Receive_DATA(reinterpret_cast<uint8_t *>(&serial_in_buffer), 1);

    //delay_ms(1000);
    CDC_Send_DATA(reinterpret_cast<const uint8_t *>("WHAT WOULD YOU LIKE TO DO? TYPE ONE OF THESE:\n\r"), 48);
    CDC_Send_DATA(reinterpret_cast<const uint8_t *>("- BEGIN TEST FLIGHT?          't'\n\r"), 36);
    CDC_Send_DATA(reinterpret_cast<const uint8_t *>("  * REMEMBER TO PLACE QUAD ON STABLE POSITION\n\r"), 48);
    CDC_Send_DATA(reinterpret_cast<const uint8_t *>("  * TEST FLIGHT BEGINS IN 20 SECONDS\n\r"), 39);
    CDC_Send_DATA(reinterpret_cast<const uint8_t *>("- READ FROM FLASH MEMORY?     'r'\n\r"), 36);
    ...
    if(serial_in_buffer == 'r'){
        CDC_Send_DATA(reinterpret_cast<const uint8_t *>("------------READING FROM BLACK BOX-------------\n\r"), 50);
        CDC_Send_DATA(reinterpret_cast<const uint8_t *>("THESE WERE WRITTEN TO MEMORY DURING LAST FLIGHT\n\r"), 50);
        CDC_Send_DATA(reinterpret_cast<const uint8_t *>("- HEIGHT?                        'ht'\n\r"), 39);
        CDC_Send_DATA(reinterpret_cast<const uint8_t *>("- ANGLE? (ROLL PITCH YAW)        'ag'\n\r"), 39);
        CDC_Send_DATA(reinterpret_cast<const uint8_t *>("- ACCELERATION? (ROLL PITCH YAW) 'ac'\n\r"), 39);
        CDC_Send_DATA(reinterpret_cast<const uint8_t *>("- ALL?                           'al'\n\r"), 39);
        CDC_Send_DATA(reinterpret_cast<const uint8_t *>("-----------------------------------------------\n\r"), 50);
...
//        }
    ...
    if(serial_in_buffer == 't'){
        ...
        CDC_Send_DATA(reinterpret_cast<const uint8_t *>("FLIGHT TESTING PROCEDURE BEGINS IN 20 SEC\n\r"), 44);
        delay_ms(10000);
