...
void BMP180_WriteReg(uint8_t reg, uint8_t value) {
    ...
    buf[0] = reg;
    buf[1] = value;
    write_data_async(&BMP180,BMP180_ADDR,buf[0],&buf[1], NULL, false);
}
...
uint8_t BMP180_ReadReg(uint8_t reg) {
    ...
    write_data_async(&BMP180, BMP180_ADDR, reg, 0, NULL, false);
    // Read register value
    read_data_async(&BMP180,BMP180_ADDR, reg, 1, &value, NULL, false);
...
void BMP180_ReadCalibration(void) {
    ...
    write_data_async(&BMP180, BMP180_ADDR,buffer[0], 0, NULL, false); // Send calibration first register address
    read_data_async(&BMP180, BMP180_ADDR,buffer[0], BMP180_PROM_DATA_LEN, buffer, NULL, false);
...
BMP180_RESULT BMP180_Read_UT(uint16_t *UT) {
    ...
    if (write_data_async(&BMP180, BMP180_ADDR, buf[0], 0, NULL, false)) {
        *UT = 0;
        return BMP180_ERROR;
    }
    // Read ADC MSB and LSB
    if (!read_data_async(&BMP180, BMP180_ADDR,buf[0], 2, buf, NULL, false)) {
        *UT = (buf[0] << 8) | buf[1];
...
BMP180_RESULT BMP180_Read_PT(uint32_t *PT, uint8_t oss) {
    ...
    if (!read_data_async(&BMP180, BMP180_ADDR,buf[0], 3, buf, NULL, false)) {
