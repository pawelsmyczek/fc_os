...
static void BMP180_WriteReg(uint8_t reg, uint8_t value) {
    write_data_async(&BMP180,BMP180_ADDR_WRITE,reg, &value, NULL, true);
}



static uint8_t BMP180_ReadReg(uint8_t reg) {
    uint8_t value;
...
: i2c(i2c_)
{
    ...
    bmp_state = START_TEMP_READ;
    mode = BMP180_UHIRES;
    for(uint8_t i = 0; i < PRESS_SAMPLE_COUNT; i++) pressure_average_buffer[i] = 0;
}
...
void BMP_180::read_calibration(void)
{
...
void BMP_180::temperature_calculate(void)
{
    uint16_t uncompensated_temp = 0;
    /*TEMPERATURE CALC*/
    uncompensated_temp = (temperature_buffer[0] << 8) | temperature_buffer[1];
...
void BMP_180::bmp180_update(void)
{
    ...
    if(now_ms > next_update_ms){
        ...
        dt = (float)(next_update_ms - now_ms)/1000.0f;
    }
